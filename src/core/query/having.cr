...
struct Core::Query(Schema)
  # :nodoc:
  alias HavingTuple = NamedTuple(clause: String, params: Array(Param)?)
  ...
  def having(or = false, **having)
    ...
    having(group.map(&.[:clause]).join(" AND "), group.flat_map(&.[:params]).compact, or: or)
  end
  ...
  macro having_clause
    ...
    if having_values.any?
      ...
      params.concat(having_values.flat_map(&.[:params]).compact)
    end
    ...
    if or_having_values.any?
      ...
      params.concat(or_having_values.flat_map(&.[:params]).compact)
    end
  end
end
